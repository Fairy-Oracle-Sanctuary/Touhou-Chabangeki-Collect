...
/**
 * 鉴权弹窗：登录 / 注册（邮箱+密码表单）/ 找回密码 / 天机阁通行证（SSO 跳转）
 *
 * props:
 *  - onClose()
 ...
 *  - onSSO()  天机阁通行证：跳转统一登录页（sso.sofurry.cloud）
 */
export default function AuthModal({ onClose, onSignIn, onSignUp, onReset, onSSO }) {
    ...
    const goSSO = async () => {
        ...
        try {
            ...
            setSsoBusy(false);
            setError("跳转统一登录失败，请稍后再试");
            console.error("[auth] SSO 跳转失败:", e);
        }
    };
    ...
    return (
        <Modal title={titles[mode]} icon={icons[mode]} onClose={onClose}>
            ...
            <button className="btn btn-block" disabled={ssoBusy} onClick={goSSO}>
                {ssoBusy ? "跳转中…" : "使用天机阁通行证登录"}
            </button>
        </Modal>
    );
}
